...
class ShinigamiView(ttk.Frame):
    ...
    def scrape_chapter(self):
        ...
        self.log(f"[START] Scraping Shinigami: {url}")
        shinigami_scrape_series(url, log=self.log)
        self.reload_txt_files()
        self.log("[DONE] Scrape chapter list selesai.\n")

    def scrape_images(self):
        selected = self.txt_var.get().strip()
        if "<Tidak ada txt>" in selected:
            self.log("[ERROR] Tidak ada file .txt")
            return

        self.log(f"[START] Download images dari: {selected}")
        shinigami_download_batch(selected, log=self.log)
