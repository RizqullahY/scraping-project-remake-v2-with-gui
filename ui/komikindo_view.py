...
from modules.komikindo_chapter import scrape_judulseries
from modules.komikindo_image import scrape_images_batch, TXT_DIR


class KomikindoView(ttk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller

        ttk.Label(self, text="Komikindo Scraper", font=("Segoe UI", 18, "bold")).pack(pady=15)

        # URL Input
        self.url_entry = ttk.Entry(self, width=60)
        self.url_entry.pack(pady=5)
        self.url_entry.insert(0, "Masukkan URL Komikindo…")
