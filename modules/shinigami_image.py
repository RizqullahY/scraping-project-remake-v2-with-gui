...
def download_image(url, path, log=None):
    if log is None:
        log = print
    try:
...
def scrape_chapter(uuid, out_folder, log=None):
    if log is None:
        log = print
    ...
    log(f"[CHAPTER] {uuid} | {len(images)} gambar")
...
def shinigami_download_batch(txt_file, log=None):
    if log is None:
        log = print
